...
interface CardProps {
  type: string; // company or miniBusiness
  title: string;
  desc: string;
...
export default function RegistrationItem({
  ...
  const handleNext = async () => {
    ...
    sessionStorage.setItem("regNumber", bnNumber);
    try {
    ...
    >
      <div className="p-6 text-center">
        <input
          ...
          checked={selectedCard === type}
          onChange={() => setSelectedCard(type)}
        />
        <section className="pt-5">
          ...
          <p className="text-sm text-gray-500 font-[Onest] mt-2">{desc}</p>
        </section>
      </div>
      {isActive && (
        <motion.div
        ...
        >
          <InputItem
            ...
            placeholder={
              type === "company"
                ? "Please input a valid RC Number (RC-XXXXXXX)"
                : "Please input a valid BN Number (BN-XXXXXXX)"
            }
            title={type === "company" ? "RC Number" : "BN Number"}
            errorMssg={error}
            value={bnNumber}
            handleChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              setBnNumber(e.target.value)
            }
            fieldsetStyle="bg-white p-4 w-full"
            labelStyle="w-40 text-gray-600"
          />
          <div
            className="flex bg-green-900 mt-3 mr-8 px-7 py-1.5 rounded-lg cursor-pointer"
            onClick={(e) => {
              e.stopPropagation(); // CRITICAL: Prevents the parent card's onClick from firing
              handleNext();
            }}
          >
            <button
              className="text-white font-[Nunito] cursor-pointer"
              type="button"
